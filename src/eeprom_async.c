...
{
    if (!e || !e->init || !len) return false;

    static uint8_t tx[1];
    tx[0] = start_addr;

    static eeproma_ctx_t c;
    c.e = e; c.buf = buf; c.len = len; c.cb = cb; c.ctx = ctx;

    i2c_transaction_t t = {
        .address = e->address, .tx_buf = tx, .tx_len = 1,
        .rx_buf = buf, .rx_len = len, .cb = eeproma_read_cb, .context = &c
    };
